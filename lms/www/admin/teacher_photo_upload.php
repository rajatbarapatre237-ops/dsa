...
if (isset($_POST['btn_upload']))
  {
    ...
    $stmt = $dbc->prepare("INSERT INTO teacher_photo (img_naam,img_name, img_type, img_path, img_sub) VALUES (:inaam,:iname, :itype, :ipath, :isub) ");
    $stmt->bindValue(':inaam', $name);
   
    $stmt->bindValue(':iname', $filename);
    $stmt->bindValue('itype', $filetype);
    $stmt->bindValue('ipath', $filepath);
    // $stmt->bindValue('ititle', $filetitle);
    $stmt->bindValue('isub', $sub);
    if ($stmt->execute())
      {
        header('Location: teacher_photo_upload.php?success=yes&title=' . $name);
      }
    else
      {
        header('Location: teacher_photo_upload.php?success=no');
      }
  }
?>
    ...
    <div class="pagetitle">
        ...
        <nav>
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Upload Teacher Photo</li>
          </ol>
        </nav>
      </div>
            ...
            <?php } }?>
              ...
              <form class="row g-3"action="<?= $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data" data-abide>
                ...
                <label>Enter Teacher's Name</label>
                  <input type="text" class="form-control" name="img_naam" placeholder="Teacher's Name" required>
                </div>
                <div class="col-md-6">
                <label>Enter Subject</label>
                  <input type="text" class="form-control" name="img_sub" placeholder="Enter Subject" required>
                </div>
