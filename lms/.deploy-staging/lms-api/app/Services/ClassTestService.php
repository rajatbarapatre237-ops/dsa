...
class ClassTestService
{
        ...
        return DB::table('subject as s')
            ->join('courses_subjects as cs', function ($j) {
                $j->on('cs.course_name', '=', 's.course_name')
                    ->whereRaw('LOWER(TRIM(cs.subject_name)) = LOWER(TRIM(s.subject_name))');
            })
    ...
    public function teacherAssignedToCourse(string $email, int $courseId): bool
    {
        $cn = $this->courseName($courseId);
        if (! $cn) {
            return false;
        ...
        return DB::table('course_assign')
            ->where('email', $email)
            ->where('course', $cn)
            ->exists();
    }

    public function canEnterMarks(string $email, object $test): bool
    {
        if (strcasecmp((string) ($test->created_by ?? ''), $email) === 0) {
            return true;
        }
        ...
        if ($subjectId > 0 && $this->teacherAssigned($email, $courseId, $subjectId)) {
            return true;
        }

        if (! $this->teacherAssignedToCourse($email, $courseId)) {
            return false;
        }

        $subjectMissing = $subjectId <= 0
            || ! DB::table('subject')->where('id', $subjectId)->exists();

        return $subjectMissing || (($test->created_by_role ?? '') === 'admin');
    }
    ...
    public function studentsMarks(int $testId, string $email, ?string $sessionName, ?string $batch): array
    {
        ...
        if (! $this->canEnterMarks($email, $test)) {
            return ['ok' => false, 'error' => 'You are not allowed to enter marks for this test'];
        }
