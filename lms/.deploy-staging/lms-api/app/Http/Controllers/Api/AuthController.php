...
class AuthController extends Controller
{
    ...
    private function parseStudentNumericId(string $raw): ?string
    {
        $raw = strtoupper(trim($raw));
        if ($raw === '' || str_starts_with($raw, 'ACE')) {
            return null;
        }
        if (str_starts_with($raw, 'DSA')) {
            $numeric = substr($raw, 3);
    ...
    private function invalidStudentIdResponse(): JsonResponse
    {
        return response()->json([
            'status' => 'error',
            'message' => 'Use DSA615 or 615 as student ID',
        ], 422);
    }

    private function formatStudentSid(string $numericId): string
    {
        return 'DSA'.trim($numericId);
    }
    ...
    public function studentLogin(Request $request): JsonResponse
    {
        ...
        $numericId = $this->parseStudentNumericId($raw);
        if ($numericId === null) {
            return $this->invalidStudentIdResponse();
        }
        ...
        $sid = $this->formatStudentSid($numericId);
        $token = $this->tokens->issue([
            'role' => 'student',
            'sid' => $sid,
            'id' => $numericId,
    ...
    public function parentLogin(Request $request): JsonResponse
    {
        ...
        $studentId = $this->parseStudentNumericId($data['student_id']);
        if ($studentId === null) {
            return $this->invalidStudentIdResponse();
        }
        ...
        return response()->json([
            ...
            'user' => [
                'role' => 'parent',
                'student_id' => $this->formatStudentSid($studentId),
