...
import { resolveCourseMeta, resolveStudentId } from './StudentHubUi';
...
export function StudentContextCard({
  ...
  const displayName = (name ?? fallbackName).trim() || fallbackName;
  const initial = displayName.charAt(0).toUpperCase();

  const { course: resolvedCourse, batch: resolvedBatch } = resolveCourseMeta({
    course,
    batch,
    profile,
    attendanceSummary,
  ...
  const resolvedId = studentId ?? resolveStudentId(profile);
  const courseParts = [resolvedCourse, resolvedBatch].filter(Boolean);
  ...
  return (
    <View style={styles.contextCard}>
      ...
      <View style={styles.contextBody}>
        <Text style={styles.contextName}>{displayName}</Text>
        {resolvedId ? <Text style={styles.contextId}>ID {resolvedId}</Text> : null}
        <Text style={styles.contextMeta}>
          {courseParts.length > 0 ? courseParts.join(' · ') : 'Course details unavailable'}
        </Text>
      </View>
      <View style={styles.contextBadge}>
        <AppIcon name="school-outline" size={16} color={PRIMARY} />
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  contextId: { fontSize: 12, fontWeight: '700', color: PRIMARY, marginTop: 3 },
  contextMeta: { fontSize: 12, color: theme.muted, marginTop: 2 },
  contextBadge: {
    width: 36,
    height: 36,
    borderRadius: 12,
